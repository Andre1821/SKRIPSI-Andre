...
import routing.*;
import routing.community.ChitChatPrioritized1;
 ...
 */
public class MessageStatsReportMod1 extends Report implements MessageListener {
    ...
    public void messageTransferred(Message m, DTNHost from, DTNHost to,
        ...
        if (to.getSocialProfile().contains(topic.getKey()) || m.getId().contains("S")) {
            if (nrofMsgInterest.containsKey(m.getId())) {
                Set<DTNHost> host = new HashSet<>(nrofMsgInterest.get(m.getId()));
                host.add(to);
